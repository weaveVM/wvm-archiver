...
pub async fn handle_info() -> Json<Value> {
    ...
    let last_backfill = ps_get_blocks_extremes("backfill", true).await;
    ...
    let stats_res: InfoServerResponse = InfoServerResponse::new(first_livesync, last_livesync, first_backfill, last_backfill).await;
